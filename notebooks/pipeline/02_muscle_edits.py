...
Run this after `01_non_muscle_edits.py`. It applies muscle-specific edits
(muscle model conversion, mesh registration, via-point path redefinition,
parameter updates) and writes final unilateral outputs.
"""
...
print("Converted muscles to Millard2012EquilibriumMuscle")

# %% Register meshes and load Young attachment points
# Align foot mesh without phalanges
...
# Register all bone meshes from Young to Johnson coordinate systems
...
print(f"Loaded {len(young_attachments)} attachment points from Young model")

# %% Replace wrapping surfaces with via-point paths from Young model
...
for _i in range(_muscles.getSize()):
    ...
    # Look up attachment points for this muscle
    rows = young_attachments.filter(
        pl.col("Abbreviation") == _muscle_name.split("R_")[-1]
    )
    index = 1
    for row in rows.iter_rows(named=True):
        ...
        point_name = (
            _muscle_name + "_" + frame + "_" + row["Type"].lower() + "_" + str(index)
        )
    ...
    # Remove original via points
    for j in range(existing_path_points - 1, -1, -1):
        path_points.remove(j)

print("Replaced wrapping surfaces with via-point paths")

# %% Apply Johnson 2011 muscle parameters and optimized TSL values
...
for _i in range(_muscles.getSize()):
    ...
    # Set architectural parameters from Johnson 2011
    params = johnson_params.row(
    ...
    if params is not None:
        ...
        _muscle.setPennationAngleAtOptimalFiberLength(
            params["θ0 (deg)"] * np.pi / 180
        )

    # Set tendon slack length from optimization results
    tsl_params = tsl_df.row(
        by_predicate=pl.col("Abbreviation") == _muscle_name, named=True
    )
